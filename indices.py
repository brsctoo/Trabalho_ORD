...
def criar_indices():
    ...
    while buffer != '':
        ...
        if(buffer[0] != '*'):
            ...
            # Está na lista
            if idx_gen != -1:
                registro_inv[1] = indice_gen[idx_gen][1] # O novo aponta para o velho -> Entra na cabeça da lista
                indice_gen[idx_gen][1] = rnn_atual # O genêro no indice_gen agora aponta pro o novo
            else:
                # Não está na lista
            ...
            # Está na lista
            if idx_pub != -1:
                registro_inv[2] = indice_pub[idx_pub][1] # O novo aponta para o velho -> Entra na cabeça da lista
                indice_pub[idx_pub][1] = rnn_atual # O genêro no indice_gen agora aponta pro o novo
            else:
                # Não está na lista
